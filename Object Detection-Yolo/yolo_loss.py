...
class YoloLoss(nn.Module):
    ...
    def get_regression_loss(self, box_pred_response, box_target_response):   
        ...
        centre_l=F.mse_loss(box_pred_response[:,:2],box_target_response[:,:2],reduction='sum') 
        wh_l=F.mse_loss(torch.sqrt(box_pred_response[:,2:4]),torch.sqrt(box_target_response[:,2:4]),reduction='sum')
        reg_loss=centre_l+wh_l
        ##### CODE #####
    ...
    def get_no_object_loss(self, target_tensor, pred_tensor, no_object_mask):
        ...
        not_contain_object_pred=pred_tensor[no_object_mask].reshape(-1,30)
        not_contain_object_target=target_tensor[no_object_mask].reshape(-1,30)
        element_mask=torch.zeros(not_contain_object_pred.size(),dtype=torch.uint8).cuda()
        element_mask[:,4]=1
        element_mask[:,9]=1
        #print(element_mask)
        not_contain_object_classpred=not_contain_object_pred[element_mask]
        not_contain_object_classtarget=not_contain_object_target[element_mask]
        no_object_loss=F.mse_loss(not_contain_object_classpred,not_contain_object_classtarget,reduction='sum')
    ...
    def find_best_iou_boxes(self, box_target, box_pred):
        ...
        for i in range(0,box_target.size(0),2):
            box_p=box_pred[i:i+2]
            box_pprocess_xywh=torch.Tensor(box_p.size())
            '''x, y = x/S - 0.5*w, y/S - 0.5*h ; w, h = x/S + 0.5*w, y/S + 0.5*h'''
            box_pprocess_xywh[:,:2]=(box_p[:,:2]/14)-(0.5*box_p[:,2:4])
            box_pprocess_xywh[:,2:4]=(box_p[:,:2]/14)+(0.5*box_p[:,2:4])
            box_t=box_target[i].reshape(-1,5)
            box_tprocess_xywh=torch.Tensor(box_t.size())
            '''x, y = x/S - 0.5*w, y/S - 0.5*h ; w, h = x/S + 0.5*w, y/S + 0.5*h'''
            box_tprocess_xywh[:,:2]=(box_t[:,:2]/14)-(0.5*box_t[:,2:4])
            box_tprocess_xywh[:,2:4]=(box_t[:,:2]/14)+(0.5*box_t[:,2:4])
            
            result_iou = self.compute_iou(box_pprocess_xywh[:,:4],box_tprocess_xywh[:,:4])
            iou_max,iou_index=torch.max(result_iou,0)
            contains_object_response_mask[i+iou_index.data.cuda()]=1
            box_target_iou[i+iou_index.data.cuda(),torch.cuda.LongTensor([4])]=iou_max.data.cuda()
    ...
    def forward(self, pred_tensor,target_tensor):
        ...
        ##### CODE #####
        box_prediction_response=box_pred[contains_object_response_mask].reshape(-1,5)
        box_target_response_iou=box_target_iou[contains_object_response_mask].reshape(-1,5)
        box_target_response=box_target[contains_object_response_mask].reshape(-1,5)
